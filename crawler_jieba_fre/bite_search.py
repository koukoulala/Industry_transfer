...
class BiteSearch(object):

    search_page_url="http://search.chinabyte.com/searchchianbyte.do?wd={}&title=5.0&content=5.0&page={}"
    ...
    def search_page(key_word, page):
        ...
        try:
            ...
            url = BiteSearch.search_page_url.format(key_word_quote,page)
            failure = 0
            ...
            while len(url) > 0 and failure < 10:
                ...
                if r.status_code == 200:
                    ...
                    result_links = bs_obj.find("ul",class_="pr50 Information").findAll("li")
                    for result in result_links:
                        try:
                            item_ins = deepcopy(BiteSearch.item)
                            title_node = result.find("div",class_="f14 fb new_blue mb5")
                            item_ins['link'] =title_node.find("a").get("href")
                            item_ins['title'] = title_node.find("a").get_text()
                            item_ins['abstract'] = result.find("div",class_="f14 deepgray mb5 t2").get_text()
                            item_ins["date"] = result.findAll("span",class_="mr30")[1].get_text()
                            ...
                            logging.error("Parse 比特网 result error. ErrorMsg: %s" % str(e))
                    break
                else:
                    failure += 2
            ...
            logging.error("Get 比特网 search result error. ErrorMsg: %s" % str(e))
        return result_list


if __name__ == "__main__":
    for i in range(1, 10):
        url_list = BiteSearch.search_page("广州 服务业 转移", page=i)  # 已经测试过了,运行正常
