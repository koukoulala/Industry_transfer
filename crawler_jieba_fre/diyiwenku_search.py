...
class diyiwenkuSearch(object):

    search_page_url="http://s.wenku1.com/cse/search?q={}&p={}&s=5400272476022044305&entry=1"
    ...
    def search_page(key_word, page):
        ...
        try:
            ...
            url = diyiwenkuSearch.search_page_url.format(key_word_quote,page)
            failure = 0
            ...
            while len(url) > 0 and failure < 10:
                ...
                if r.status_code == 200:
                    ...
                    result_links = bs_obj.find("div", class_="content-main").findAll("div", class_="result f s0")
                    for result in result_links:
                        try:
                            item_ins = deepcopy(diyiwenkuSearch.item)
                            ...
                            item_ins["date"] = result.find("span", class_="c-showurl").get_text()
                            ...
                            logging.error("Parse 第一文库网 result error. ErrorMsg: %s" % str(e))
                    break
                else:
                    failure += 2
            ...
            logging.error("Get 第一文库网 search result error. ErrorMsg: %s" % str(e))
        return result_list


if __name__ == "__main__":
    for i in range(1, 10):
        url_list = diyiwenkuSearch.search_page("广州 服务业 转移", page=i)  # 已经测试过了,运行正常
